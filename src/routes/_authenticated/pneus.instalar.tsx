  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 pb-6">
        ...
        <CardContent className="p-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <MoneyInput value={valor} onValueChange={setValor} className="rounded-xl" />
            </div>
          </div>

          <p className="text-[11px] text-muted-foreground -mt-1">
            {posicao.includes(",")
              ? "O valor informado será lançado para cada pneu individualmente nas Despesas."
              : "Quando há valor, lançamos automaticamente em Despesas → Pneu."}
          </p>

          <div>
            <Label className="flex items-center gap-1">
              <Camera className="h-4 w-4" /> Foto (cupom/nota)
            </Label>
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    toast.error(e?.message ?? "Câmera indisponível");
                  }
                }}
      ...
      <Button
        type="submit"
        size="lg"
        className="w-full rounded-xl active-scale bg-primary text-primary-foreground hover:bg-primary/90"
        disabled={submitting}
      >
        {submitting
          ? "Salvando..."
          : posicao.includes(",")
            ? `Instalar ${posicao.split(",").length} Pneus`
            : "Instalar pneu"}
      </Button>
    </form>
  );
